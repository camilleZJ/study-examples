 ...
 */
// $CI =& get_instance();
// $CI->config->load('aliconf',TRUE);
// $CI->load->library('AlipayTradeWapPayRequest');
...
class AlipayTradeService {
	...
	function wapPay($builder,$return_url,$notify_url) {
		...
		$this->writeLog($biz_content); //打印业务参数
		
		//$this->CI->load->library('alipay/AlipayTradeWapPayRequest','','alipayRequest');
	...
	function aopclientRequestExecute($request,$ispage=false) {
		...
		//$this->CI->load->library('alipay/AopClient','','aop');
		$aop = new AopClient ();
	...
	function writeLog($text) {
		...
		//$text = characet ( $text );
		file_put_contents ( APPPATH."logs/alipay_log.txt", date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
